...
const dislikeAReviewHandler = async (
  ...
  try {
    ...
    if (!review.likes?.includes(userId)) {
      return APIResponse.error("You haven't liked this review yet", 400).send(
        res
      );
    }
    ...
    return APIResponse.success(
      { message: "Review disliked", data: updatedReview },
      200
    ).send(res);
  } catch (error) {
...
export default dislikeAReviewHandler;
